...
// ── Ingest filters (shared by webhook + poller paths) ────────
...
/**
 * Build Vectorize vector ID for an issue / PR top-level comment.
 * Deterministic SHA-256-based ID under the "ic" prefix.
 */
export function issueCommentVectorId(
  repo: string,
  commentId: number,
): Promise<string> {
  return stableVectorId("ic", repo, String(commentId));
}

/**
 * Build Vectorize vector ID for a PR review (approve / request_changes / comment body).
 * Deterministic SHA-256-based ID under the "pv" prefix.
 */
export function prReviewVectorId(
  repo: string,
  reviewId: number,
): Promise<string> {
  return stableVectorId("pv", repo, String(reviewId));
}

/**
 * Build Vectorize vector ID for a PR inline review comment (per-line diff comment).
 * Deterministic SHA-256-based ID under the "pc" prefix.
 */
export function prReviewCommentVectorId(
  repo: string,
  commentId: number,
): Promise<string> {
  return stableVectorId("pc", repo, String(commentId));
}

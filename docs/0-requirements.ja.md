...
2. structured params から Vectorize metadata filter を構築（repo, state, type, milestone は pre-filter）
3. labels / assignee フィルタ指定時は内部 topK をオーバーフェッチ（requestedTopK × 5, max 50）
4. embedding + filter で Vectorize を引く
5. labels（AND ロジック、個別フィールド + CSV）と assignee を post-filter
6. requestedTopK にトリムして structured context と共に返す
...
- issue / pull request / release / documentation / commit diff を semantic search する
